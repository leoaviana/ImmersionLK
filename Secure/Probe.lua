...
function L:CreateProbe(owner, object, state, name, script)
	...
	local probe = setmetatable(CreateFrame('Frame', name, object, 'SecureHandlerStateTemplate, SecureHandlerShowHideTemplate, SecureHandlerEnterLeaveTemplate'), Probe_MT)
	probe:SetType(state)
	probe:SetOwner(owner)
	probe:SetObject(object)
	probe:SetResponseScript(script) 
	probe:SetAttribute("incombat", InCombatLockdown() and 1 or 0)
	
	RegisterStateDriver(probe, 'combat', '[combat] 1; 0')
	probe:SetAttribute('_onstate-combat', [[
...
function L:InsecureHookShowHide(owner, object)
	...
	ImmersionAPI.SetShown(owner, object:IsShown())
end
...
----------------------------
SCRIPT = {
	show = [[_=(owner:GetAttribute('pc') or 0)+1 owner:SetAttribute('pc',_) if tonumber(self:GetAttribute("incombat")) ~= 0 then owner:Hide() owner:Show() end]],
	hide = [[_=(owner:GetAttribute('pc') or 1)-1 owner:SetAttribute('pc',_) if _ < 1 then if tonumber(self:GetAttribute("incombat")) ~= 0 then owner:Hide() end end]],
