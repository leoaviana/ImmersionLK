...
function L:CreateFrame(object, name, parent, xml, blueprint, recursive)
	----------------------------------
...
function L:BuildFrame(frame, blueprint, recursive)
...
SETUP = {
----------------------------------
	[1] = function(frame, blueprint) L:BuildFrame(frame, blueprint, true) end,
	ID 	= function(region, ...) region:SetID(...) end,
	--- Texture
	Atlas 	= function(texture, ...) ImmersionAPI.SetAtlas(texture,...) end,
	...
	Background = function(frame, backdrop) L.SecMedia:SetBackdrop(frame, backdrop) end,
	...
	Probe 	= function(region, ...) region.probe = L:CreateProbe(region, ...) end,
	...
	-- Mixin 
	Mixin 	= function(region, ...) L:SecApplyMixin(region, nil, ...) end,
	MixinNormal = function(region, ...) L:SecApplyMixin(region, L.SecUtils.MixinNormal, ...) end,
	--- Multiple runs
...
L.SecFunctions = SETUP
L.SecRegion = REGION
-- Shortcut for mixin with scripts, since it's expected to be used frequently.
L.SecMixin = SETUP.Mixin

----------------------------------
L.SecFrameRegistry = {}
function L:OnFrameCreated(frame)
	assert(frame)
	self.SecFrameRegistry[frame] = true
end

function L:RemoveRegisteredFrame(frame)
	assert(frame and self.SecFrameRegistry[frame], 'The supplied frame is not registered.')
	self.SecFrameRegistry[frame] = nil
end