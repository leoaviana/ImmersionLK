...
frame.oldp = frame:GetParent()
frame:SetParent(nil);

inspector.oldp = frame:GetParent()
inspector:SetParent(nil); 
...
--titles:RegisterUnitEvent('UNIT_QUEST_LOG_CHANGED', 'player') I think it should be easy enough to fix.
...
function frame:ADDON_LOADED(name)
	if name == _ then
		...
		-- Hide portrait 
		ImmersionAPI.SetShown(talkbox.PortraitFrame, not L('disableportrait'))
		ImmersionAPI.SetShown(talkbox.MainFrame.Model.PortraitBG, not L('disableportrait'))

		-- Show solid background
		ImmersionAPI.SetShown(talkbox.BackgroundFrame.SolidBackground, L('solidbackground'))
...
text:SetPoint('TOPLEFT', talkbox.PortraitFrame.Portrait, 'TOPRIGHT', 2, -35)

L.Mixin(text, L.TextMixin) -- see Mixins\Text.lua
...
text:SetFontObjectsToTry(GameFontNormalLarge, GameFontNormalLarge, GameFontNormalLarge)
...
talkbox.TextFrame.SpeechProgress:SetFont("Fonts\\FRIZQT__.TTF", 16, '') -- NFSH used to be morpheus font.
