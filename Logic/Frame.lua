...
function Frame:PlayIntro(event, freeFloating)
	...
	-- updates the frame bindings before showing it.
	L:GetControlHandle():UpdateBindings()
...
-- if ConsolePortUI is loaded, then we will setup the ImmersionFrame as a ConsolePortUI Frame.
-- By doing that, we will not need SetPropagateKeyboardInput (we don't have it on 3.3.5a unfortunately) and also OnKeyDown/OnKeyUp events.
-- That also means ImmersionFrame will be shown during combat avoiding UI taint.

L:RegisterFrame(L.frame, "IMMERSIONADDON", nil, false, false)
L:CreateProbe(L.frame, QuestFrame, 'showhide')
L:CreateProbe(L.frame, GossipFrame, 'showhide')